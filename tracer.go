...
func (l *Tracer) NewLogger(id string, appName, uuid string, maxTracers int, appNameColour string) (*Logger, error) {
...
func (l *Logger) NewTrace(id string, traceName, traceID string, maxTraceCount int, debugColour string) *Trace {
	trace := &Trace{
		...
		Messages:            make([]*Message, 0, maxTraceCount),
		zLog:                l.zLog,
...
func (t *Trace) newLog(level zerolog.Level, logMessage, messageType string) {
	...
	text := fmt.Sprintf("[app=%-6s] | trace=%-15s | %-10s", strings.ToUpper(t.LoggerName), t.Name, newMessage.Message)
